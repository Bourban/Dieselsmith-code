...
public static class GraphicsRaycast
{
    ...
    {
        Camera rayCam;
        UniversalAdditionalCameraData rayCamData;

        // TODO: Make this varitable depending on graphics pipeline using this:
        ...
        if (!RayCamObj)
        {
            ...
            rayCamData = RayCamObj.AddComponent<UniversalAdditionalCameraData>();
        }
        else
        {
            rayCam = RayCamObj.GetComponent<Camera>();
            rayCamData = RayCamObj.GetComponent<UniversalAdditionalCameraData>();
        }

        RayCamObj.hideFlags = HideFlags.HideAndDontSave;

        rayCamData.SetRenderer(1);
